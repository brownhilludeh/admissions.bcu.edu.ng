...
@section('content')
<form action="{{ route('msg_send') }}" class="validate" autocomplete="off" method="post" accept-charset="utf-8">
    <div class="row">
        <div class="col-md-8">
                ...
                <div class="card-body border">
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-group col-md-12">
                            ...
                            <textarea class="form-control summernote" id="editor" name="body" required>{{ old('body') }}</textarea>
                        </div>
	...
	function getUsers(type) {
		$.ajax({
			...
			success: function(data) {
                ...
                $('select[name=user_id]').append("<option value='"+val['id']+"'>"+val['first_name']+" "+val['last_name']+", ("+val['email']+")</option>");
                });
		...
		if ($("#user_id").val() == "all") {
			...
			$.ajax({
				...
				success: function(data) {
                    ...
                    .append('<div class="col-md-12">'+
                        '<label class="c-container">'+
                            '<input type="checkbox" value="'+val['id']+'" name="users[]" checked="true">'+val['last_name']+' - '+val['email']+'<span class="checkmark"></span>'+'</label>'+                        '</div>');
                    });
