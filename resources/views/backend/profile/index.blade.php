...
<div class="tab-content">
  ...
  <div id="myProfile" class="tab-pane fade show">
    <div class="row justify-content-evenly">
      <div class="col-md-4">
        <div class="card" class="rounded-4">
          <div class="card-body box-profile rounded-4">
            <div class="text-center">
              <img class="profile-user-img img-fluid img-circle" src="{{ asset('public/uploads/images/'.$profile->image )  }}" style="width: 150px; border-radius: 50%" alt="no image">
            </div>
            <h3 class="profile-username text-center">{{$profile->name ?? 'N/A'}}</h3>
            <p class=" text-center">{{$profile->email ?? 'N/A'}}</p>
            <ul class="list-group list-group-unbordered mb-3">
              <li class="list-group-item">
                <b>{{__('User ID')}}</b> <a class="float-right">{{$profile->username ?? 'N/A'}}</a>
              </li>
              <li class="list-group-item">
                <b>{{__('Account Type') }}</b> <a class="float-right">{{$profile->user_type ?? 'N/A'}}</a>
              </li>
              <li class="list-group-item">
                <b>Account Status</b> <a class="float-right">
                  @if ($profile->status == 1) <span class="text-success">Active</span>
                  @else <span class="text-danger">Pending</span>
                  @endif</a>
              </li>
              <li class="list-group-item">
                <b>{{__('Phone')}}</b> <a class="float-right">{{$profile->phone ?? 'N/A'}}</a>
              </li>
            </ul>
          </div>
        </div>
              ...
              <tbody style="text-align: center;">
                ...
                <tr class="text-center">
                  ...
                  <td>
                    ...
                    <a href="{{ url('password.request') }}" class="text-orange btn btn-orange">Reset Password link</a>
                    @else
                    <a href="{{ url('profile.update') }}" class="text-orange btn btn-orange">Link an email address</a>
                    @endif
                  </td>
                </tr>
              </tbody>
  ...
  <div class="row justify-content-center">
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          <form action="{{ url('updatePassword', Auth::user()->id) }}" class="validate" enctype="multipart/form-data" method="post" accept-charset="utf-8">
            @csrf
    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <form method="post" class="bp-submit" autocomplete="on" action="{{ url('upload_logo') }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
