            ...
            <div class="card">
                <div class="card-header justify-content-between">
                    ...
                    <select id="user_type" class="custom-select" onchange="showUser(this);">
                        ...
                        <option @if (old('Teacher') == 'Teacher') selected @endif value="Teacher" disabled>{{ __('Teacher') }}</option>
                        <option @if (old('Student') == 'Student') selected @endif value="Student">{{ __('Student') }}</option>
                    </select>
                    <a href="{{ route('users.create') }}" data-title="{{ __('Add New User') }}"
                        class="btn app-blue ajax-modal">{{ __('Add New User') }}</a>
                </div>
                <div class="card-body">
                        ...
                        <tbody>
                            @foreach ($users as $data)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('status', $data->id) }}" class="btn btn-{{ $data->status ? 'success' : 'danger'}}">
                                            {{ $data->status ? 'Active' : 'Inactive' }}
                                        </a>
                                    </td>
                                    <td>
                                        <form action="{{ route('users.destroy', $data->id) }}" method="post">
                                            <a href="{{ route('users.edit', $data->id) }}" data-title="{{ __('Edit User Information') }}"
                                                class="orange ajax-modal">
